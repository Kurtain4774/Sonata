...
const cache = new Map(); // spotifyId -> { at, data }
...
function computeStreak(recent = []) {
  ...
  // current streak: how many consecutive days ending today (or yesterday) are present
  let current = 0;
  ...
  for (let i = 0; i < 365; i++) {
    ...
    else if (i === 0) continue; // allow no-play today, check yesterday
    else break;
  }
  return { current, best };
}
...
export async function GET() {
  ...
  try {
    ...
    // Approximate total listening time: avg track ~3.5 min × number of plays (Spotify doesn't expose duration on recently-played without an extra fetch). We bias for the week window using timestamps.
    ...
    if (err instanceof SpotifyAuthError) {
      // soft-degrade; tiles will show zeros for Spotify-derived fields
    } else {
      console.warn("/api/stats/summary partial failure:", err?.message);
    }
  }
