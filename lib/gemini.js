...
function appendExcludedArtists(systemInstruction, excludedArtists) {
  if (!excludedArtists?.length) return systemInstruction;
  const artistList = excludedArtists.map((a) => String(a).trim()).filter(Boolean).join(", ");
  if (!artistList) return systemInstruction;
...
export async function getRefinedRecommendations(originalPrompt, currentTracks, followUp, excludedArtists = []) {
  ...
  const systemInstruction = appendExcludedArtists(SYSTEM_PROMPT, excludedArtists);
...
export async function getRecommendations(prompt, personalization = null, excludedSongs = [], context = null, settings = null, excludedArtists = []) {
  ...
  systemInstruction = appendExcludedArtists(systemInstruction, excludedArtists);
  if (settings) {
    const extras = [];
...
const ARTIST_GENRE_SYSTEM_PROMPT = `You are a music classifier. The user will give you a numbered list of music artists.
Return a JSON array of objects, one per artist in the same order, each with
"artist" (the artist name exactly as given) and "genre" (a single broad genre category).
Use broad genres only (Pop, Hip Hop, Rock, R&B, Country, Electronic, Indie,
Jazz, Classical, Metal, Folk, Latin, K-Pop, Soul, Reggae, Punk, Blues, etc.).
If you don't know the artist, return "Unknown" for the genre.
Return ONLY the JSON array — no explanation.`;

export async function getArtistGenresBatch(artistNames = []) {
  const names = artistNames.map((n) => String(n || "").trim()).filter(Boolean);
  if (names.length === 0) return [];
  const numbered = names.map((n, i) => `${i + 1}. ${n}`).join("\n");
  const userMessage = `Classify each of the following artists:\n${numbered}`;
  const parsed = await callGemini(ARTIST_GENRE_SYSTEM_PROMPT, userMessage);
  return parsed.filter(
    (x) => x && typeof x.artist === "string" && typeof x.genre === "string"
  );
}
