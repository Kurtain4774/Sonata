...
export async function createPlaylist(accessToken, _userId, name, description) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to create playlist: ${res.status} — ${errBody?.error?.message || JSON.stringify(errBody)}`);
  }
  return res.json();
}
...
export async function reorderPlaylistTracks(accessToken, playlistId, rangeStart, insertBefore, rangeLength = 1) {
  const res = await fetch(
    `https://api.spotify.com/v1/playlists/${playlistId}/tracks`,
    {
      method: "PUT",
  ...
  if (res.status === 401) throw new SpotifyAuthError("Spotify token rejected");
  if (!res.ok) throw new Error(`Failed to reorder tracks: ${res.status}`);
  return res.json();
}
...
export async function getFeaturedMoods(accessToken, limit = 6) {
  const url = `https://api.spotify.com/v1/browse/featured-playlists?limit=${limit}&country=US`;
  const data = await spotifyGet(accessToken, url);
  return (data.playlists?.items || [])
    ...
    .map((p) => ({
      ...
      prompt: p.description
        ? `${p.name} — ${p.description.replace(/<[^>]*>/g, "").trim()}`
        : p.name,
      count: `${p.tracks?.total ?? "?"} tracks`,
