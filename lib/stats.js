...
export async function getStatsSummary(session) {
  ...
  try {
    const artists = await getTopArtists(session.accessToken, "short_term", 50).catch(() => []);
    artistsFetched = artists.length;
    tasteTags = deriveTasteTags(artists.flatMap((a) => a.genres || []));

    if (artists.length > 0) {
      try {
        const classifications = await getArtistGenresBatch(
          artists.map((a) => a.name).filter(Boolean)
        );
        topGenres = topGenresFromGeminiClassifications(classifications);
        console.info(
          `stats: artists=${artists.length} classified=${classifications.length} topGenres=${topGenres.length}`
        );
      } catch (err) {
        console.warn("Gemini genre classification failed, falling back to Spotify genres:", err?.message);
